...
public class Emp_AddClient_DaoImpl extends HibernateUtil implements ModelDriven<Object>, SessionAware {
	...
	private static final long serialVersionUID = -4189215621576212395L;
	private static Logger logger = Logger.getLogger(Emp_AddClient_DaoImpl.class);
	...
	public List<Emp_AddClient> list(Emp_AddClient edit) {
		...
		try {
			...
			Query query = session.createQuery("From Emp_AddClient WHERE bank_id=:clid");
	...
	public Emp_AddClient eclient(Emp_AddClient eclient) {
		...
		try {
			String SQL_QUERY2 = "UPDATE Emp_AddClient eclient set " + "eclient.branch = :branch, "
					+ "eclient.firstName = :firstName, " + "eclient.middleName = :middleName, "
					+ "eclient.lastName = :lastName, " + "eclient.gender = :gender, " + "eclient.dob = :dob, "
					+ "eclient.landLine = :landLine, " + "eclient.mobile = :mobile, " + "eclient.email = :email, "
					+ "eclient.address = :address, " + "eclient.city = :city, " + "eclient.state = :state "
